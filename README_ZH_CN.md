...
- ✅ **APK一键直接解包**：支持无Root直接解包已安装应用/本地APK文件，自动合并Split APK分包、提取资源/DEX/Manifest文件、自动反编译二进制XML/ARSC资源，解包产物可按原始目录结构直接导出到本地
  - 🔧 自动加固检测：内置18+主流加固（梆梆/360/腾讯乐固等）识别，解包前自动提示加固类型与适配解包方案
  - 📂 解包产物智能分类：自动将DEX/资源/so库/Manifest/签名信息分类存放，支持按类型快速筛选浏览
  - ⚡ 批量解包支持：支持多选多个APK/已安装应用同时解包，后台异步执行不阻塞操作
  - 🔍 资源混淆自动还原：自动识别并还原主流资源混淆方案（如AndResGuard）的资源文件名，无需手动映射
  - 🔗 MCP工具联动：解包完成后可直接调用DEX字符串替换/批量重签名等MCP工具，无需重新上传文件
  - 📤 多格式导出支持：解包产物支持一键导出为ZIP压缩包、单独导出指定类型文件（DEX/资源/so库等），支持直接分享到其他应用
  - 📸 解包快照保存：自动保存每次解包记录，包含APK信息、解包时间、解包产物路径，支持随时回溯历史解包结果
  - ⚙️ 自定义解包规则：支持用户配置解包过滤规则（例如跳过so库、仅反编译Manifest/资源、跳过大文件等），大幅提升解包速度
  - ✅ 解包完整性校验：自动校验解包产物MD5/SHA1与源APK对应区块哈希一致，避免解包损坏
  - 📊 一键生成分析报告：解包完成后自动生成APK安全分析报告，包含权限风险、组件暴露风险、第三方SDK识别、加固状态等内容
  - 🧠 大体积APK分析优化：针对超过100MB的大型APK/多DEX APK自动启用分片增量分析、内存阈值动态控制，避免分析时模型OOM崩溃
  - ⏱️ 分析超时降级机制：可配置分析超时时间，超过阈值自动降级为轻量分析模式（仅返回核心信息，跳过耗资源的代码语义分析）
  - 📶 离线后台分析：大型APK分析自动转入后台离线任务队列，支持进度查询、暂停/继续，分析完成自动推送通知，不占用前台交互资源
  - 💡 轻量分析模式：支持仅提取APK基础信息/权限/组件/SDK列表，无需反编译全量代码，10秒内完成基础分析，适配快速排查场景
  - 🔁 解包/分析断点续传：任务异常中断（进程被杀/设备重启）后，下次启动自动从上次中断位置继续执行，无需重新开始
  - 🎯 混淆代码智能标记：自动识别代码混淆特征，标记混淆类/方法，支持一键关联映射文件恢复符号
  - 🛡️ 恶意代码预检测：内置恶意特征库，分析时自动标记风险代码段（挖矿/恶意扣费/隐私窃取/广告弹窗）
  - 📱 全品类APK适配：完美支持常规应用、系统应用、车载/穿戴设备/IoT设备专用APK、Split APK分包的解包分析
  - 🤝 周边工具生态联动：解包后支持一键生成Frida Hook模板、导出到JADX-GUI/APKToolBox等第三方工具、直接生成Xposed模块框架代码
  - 📈 分析进度可视化：实时显示解包/分析进度、当前执行步骤、预估剩余时间、内存占用情况，运行状态一目了然
