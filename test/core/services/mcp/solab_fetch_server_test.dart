...
import 'package:solab/core/services/mcp/solab_fetch/solab_fetch_server.dart';

void main() {
  group('Kelivo fetch MCP', () {
    late HttpServer httpServer;
    late SolabFetchMcpServerEngine engine;
    late Uri baseUri;

    setUp(() async {
      httpServer = await HttpServer.bind(InternetAddress.loopbackIPv4, 0);
      baseUri = Uri.parse('http://127.0.0.1:${httpServer.port}');
      engine = SolabFetchMcpServerEngine();
...
''');
    ...
    test('advertises one compact, bounded fetch tool', () async {
      ...
      final serverInfo = ((initialized['result'] as Map)['serverInfo'] as Map)
          .cast<String, dynamic>();
      expect(serverInfo['name'], '@kelivo/fetch');
...
Future<Map<String, dynamic>> _callFetch(
  SolabFetchMcpServerEngine engine,
  Uri url, {
