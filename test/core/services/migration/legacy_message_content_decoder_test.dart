...
void main() {
  ...
  test(
    ...
    () async {
      ...
      expect(imagePart.uri, image.path.replaceAll('\\', '/'));
      ...
      expect(filePart.uri, pdf.path.replaceAll('\\', '/'));
  ...
  test('missing local file still creates unavailable part', () async {
    ...
    final image = result.parts.single as ImagePart;
    expect(image.uri, missing.replaceAll('\\', '/'));
  ...
  test('managed Documents absolute path becomes solab-file URI', () async {
    ...
    expect(part.uri, 'solab-file:///upload/shot.png');
    ...
    expect(
      ...
      'solab-file:///upload/shot.png',
    );
  });

  test(
    'missing managed file still yields unavailable solab-file part',
    () async {
      final docs = Directory('${tempDir.path}/Documents')..createSync();
      SandboxPathResolver.debugSetDirs(docsDir: docs.path);
      addTearDown(() => SandboxPathResolver.debugSetDirs());
      ...
      expect(result.missingFiles, 1);
      final part = result.parts.single as ImagePart;
      expect(part.uri, 'solab-file:///upload/gone.png');
      expect(part.unavailable, isTrue);
    },
  );

  test('ordinary user Documents path is not claimed as solab-file', () async {
