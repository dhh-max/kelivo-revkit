...
void main() {
  group('ModelRegistry Qwen / Doubao matrix', () {
    test('Qwen vision is precise for 3.7/3.8', () {
      ...
      final q38Open = ModelRegistry.infer(
      ...
      expect(q38Open.input, contains(Modality.image));
    ...
    test('API 返回的能力元数据优先补齐未知新模型', () {
      final model = ModelRegistry.fromApiEntry(
        ...
        id: 'future-agent-model',
        displayName: 'Future Agent Model',
      );

      expect(model.input, contains(Modality.image));
      expect(model.abilities, containsAll(ModelAbility.values));
    });
