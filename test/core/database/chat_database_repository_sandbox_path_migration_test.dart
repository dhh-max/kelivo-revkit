  ...
  group('sandbox path migration version', () {
    ...
    test('kelivo-file URI stays canonical when targetRoot changes', () async {
      ...
      await repository.migrateSandboxPaths(
        targetVersion: 1,
        targetRoot: '/old-root',
        rewriteUri: (uri) => SolabFileUri.isSolabFileUri(uri) ? uri : uri,
      );

      final result = await repository.migrateSandboxPaths(
        targetVersion: 1,
        targetRoot: directory.path,
        rewriteUri: (uri) => SolabFileUri.isSolabFileUri(uri)
            ? uri
    ...
    test(
      'canonicalize rewrite persists solab-file and skips UNC I/O',
      () async {
        ...
        expect(parts[0].uri, 'solab-file:///upload/a.png');
