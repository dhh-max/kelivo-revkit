...
import 'package:solab/core/database/business_settings_merger.dart';
...
void main() {
  ...
  group('memory system v1 database (§18.2)', () {
    ...
    test('19: message_prompt_rows is an independent cache (no FK; synthetic '
        'revisions writable; deletion cleans explicitly)', () async {
      ...
      // 合成修订（context-summary-*）从未落 message_rows，schema 4 起
      // 无外键也可写入（S16 修复）。
      await database
          .into(database.messagePromptRows)
          .insert(
            MessagePromptRowsCompanion.insert(
              revisionId: 'context-summary-c1',
              conversationId: 'c1',
              payload: 'synthetic summary',
              createdAt: now,
            ),
          );
      expect(
      ...
      // 无级联：直接删 message/conversation 时 prompt 行保留（派生缓存，
      // 由 repository 的 _deleteMessages 显式清理）。
