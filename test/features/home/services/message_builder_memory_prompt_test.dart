...
void main() {
  ...
  group('§18.1 item 6 — §7.6 decision table', () {
    ...
    test('no snapshot in history → on-demand index prefix', () async {
      await seedAssistant('assistant-1');
      await putEntry(id: 'mem_01', content: 'User likes Flutter.');
      final conversation = await seedConversation('conv-1');
      ...
      expect(result.prefix, contains(MemoryPrompts.introOnDemandZh));
      expect(result.prefix, contains('<user_memory_index>'));
      expect(result.prefix, contains('- identity: 1'));
      // 防污染契约：记忆内容一律不注入，只注入索引。
      expect(result.prefix, isNot(contains('User likes Flutter.')));
      expect(result.prefix, isNot(contains('<user_memory type="identity">')));
    ...
    test('has snapshot + same hash → empty', () async {
      ...
      final hintBlock = MemoryBlockBuilder.buildMemoryHintBlock(totals);
      final hash = MemoryBlockBuilder.hashBlocks(profileBlock, hintBlock);
    ...
    test('has snapshot + different hash → re-emit index prefix', () async {
      await seedAssistant('assistant-1');
      await putEntry(id: 'mem_01', content: 'User likes Flutter.');
      final conversation = await seedConversation(
        'conv-1',
    ...
    test('deleting the last memory emits an empty index snapshot', () async {
      await seedAssistant('assistant-1');
      await putEntry(id: 'mem_01', content: 'User likes Flutter.');
      final conversation = await seedConversation('conv-1');
      await seedUserMessage(id: 'u1', conversationId: 'conv-1');
      ...
      expect(cleared.prefix, contains(MemoryPrompts.introOnDemandZh));
      expect(cleared.prefix, contains('<user_profile/>'));
      expect(cleared.prefix, contains('<user_memory_index/>'));
    ...
    test(
      ...
      () async {
        ...
        final hash = MemoryBlockBuilder.hashBlocks(
          ...
          MemoryBlockBuilder.buildMemoryHintBlock(totals),
        );
    ...
    test(
      ...
      () async {
        ...
        expect(first.prefix, contains(MemoryPrompts.introOnDemandZh));
  ...
  group('hash ordering regression (appendix item 6)', () {
    test(
      ...
      () async {
        ...
        // 新语义下 hash 只随「档案 + 类型条数」变化：内容修改不再改变 hash，
        // 因此用新增条目（条数 1 → 2）触发 hash 变化。
        await putEntry(
          id: 'mem_02',
          content: 'Another fact that changes the count.',
        );
        ...
        expect(
          result.prefix,
          contains(MemoryPrompts.introOnDemandZh),
          reason:
              'Writing injectedMemoryHash before comparing makes the index '
              're-emit branch unreachable (appendix item 6).',
        );
        expect(result.prefix, contains('- identity: 2'));
  ...
  group('§18.4 item 29 — promptContent immutability', () {
    test(
      ...
      () async {
        ...
        expect(first, contains('<user_memory_index>'));
        // 防污染契约：记忆内容不进冻结 prompt。
        expect(first, isNot(contains('User likes Flutter.')));
      },
    );
    ...
    test(
      ...
      () async {
        ...
        expect(content, contains(MemoryPrompts.introOnDemandZh));
        expect(content, contains('<user_memory_index>'));
        expect(content, isNot(contains('User likes Flutter.')));
  ...
  group('§18.4 item 30 — self-heal after truncateIndex advances', () {
    test(
      ...
      () async {
        ...
        expect(healed.prefix, contains(MemoryPrompts.introOnDemandZh));
