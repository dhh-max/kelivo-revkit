...
void main() {
  ...
  group('SandboxPathResolver.fix solab-file', () {
    test('resolves canonical URI to absolute path without FS probe', () {
      ...
      const uri = 'solab-file:///upload/missing_no_fs_probe.png';
    ...
    test('returns solab-file URI unchanged when docsDir is null', () {
      SandboxPathResolver.debugSetDirs(docsDir: null);
      const uri = 'solab-file:///images/photo.png';
      expect(SandboxPathResolver.fix(uri), uri);
    });

    test(
      'returns invalid solab-file URI unchanged (no strip/file fallback)',
      () {
        const docs = '/tmp/kelivo_docs';
        SandboxPathResolver.debugSetDirs(docsDir: docs);
        const uri = 'solab-file:not-a-valid-structure';
        expect(SandboxPathResolver.fix(uri), uri);
      },
    );
  });
  ...
  group('SandboxPathResolver.canonicalize', () {
    test('passes through remote / data / already-canonical URIs', () {
      ...
      expect(
        SandboxPathResolver.canonicalize('solab-file:///upload/a.png'),
        'solab-file:///upload/a.png',
      );
      // Cheap prefix: illegal structure still passes through.
      expect(
        SandboxPathResolver.canonicalize('solab-file:garbage'),
        'solab-file:garbage',
      );
    });
    ...
    test('does not guess external /images/ paths into solab-file', () {
      SandboxPathResolver.debugSetDirs(docsDir: '/tmp/kelivo_docs');
    ...
    test('decodes file:// percent-escapes before encoding', () {
      ...
      expect(
        ...
        'solab-file:///upload/My%20Photo.png',
      );
    });
    ...
    test('round-trips with encodeFromAbsolute helper', () {
      ...
      expect(uri, SolabFileUri.encodeFromAbsolute(abs, root: docs));
