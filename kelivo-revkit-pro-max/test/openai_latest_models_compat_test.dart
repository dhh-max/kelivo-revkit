...
import 'package:solab/core/utils/openai_model_compat.dart';
...
void main() {
  group('latest OpenAI-compatible model compatibility', () {
    test('normalizes only officially supported reasoning efforts', () {
      ...
      // Muse Spark (1.1/1.2/contributor): relays commonly reject
      // reasoning_effort for this family, so the dial is never emitted.
      expect(
        openAINormalizeReasoningEffort('high', 'meta/muse-spark-1.1'),
        'auto',
      );
      expect(
        openAINormalizeReasoningEffort('none', 'meta/muse-spark-1.2'),
        'auto',
      );
      expect(
      ...
      expect(openAISupportsNoneReasoning('meta/muse-spark-1.2'), isFalse);
      expect(openAISupportsXhighReasoning('meta/muse-spark-1.2'), isFalse);
    });
    ...
    test('Muse Spark omits reasoning_effort for relay compatibility', () async {
      final body = await _captureChatBody(
        modelId: 'meta/muse-spark-1.2',
        thinkingBudget: 128000,
      );

      // Relays (LiteLLM pre-2026-08 registry, new-api) 400-reject the
      // parameter for this family; Meta defaults the tier when absent.
      expect(body.containsKey('reasoning_effort'), isFalse);
    });
