    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          _section(L10n.tr('路由'), [
            _kv(L10n.tr('服务商'), log.provider),
            _kv(L10n.tr('Key 名称'), log.keyName),
            _kv('Key', log.keyMasked, mono: true),
            _kv(L10n.tr('模型'), log.model, mono: true, wrap: true),
            if (log.actualModel.isNotEmpty &&
                log.actualModel != log.model)
              _kv(L10n.tr('实际模型'), log.actualModel, mono: true, wrap: true),
            if (log.ruleName != null && log.ruleName!.isNotEmpty)
              _kv(L10n.tr('路由规则'), log.ruleName!),
          ]),
          _section(L10n.tr('结果'), [
            _kv(L10n.tr('状态码'), '${log.statusCode}', mono: true),
            _kv(L10n.tr('耗时'), Formatters.formatDuration(log.durationMs)),
            if (log.totalTokens > 0) ...[
              ...
              _kv('Completion tokens',
                  ...
                  mono: true),
              _kv(L10n.tr('总 tokens'), Formatters.formatNumber(log.totalTokens),
                  mono: true),
            ],
            _kv(L10n.tr('请求字节'), Formatters.formatNumber(log.requestBytes),
                mono: true),
            _kv(L10n.tr('响应字节'), Formatters.formatNumber(log.responseBytes),
                mono: true),
          ]),
          _section(L10n.tr('标记'), [
            _kv(L10n.tr('流式'), log.streaming ? L10n.tr('是') : L10n.tr('否')),
            _kv(L10n.tr('重试次数'), '${log.retries}'),
            _kv(L10n.tr('命中缓存'), log.cached ? L10n.tr('是') : L10n.tr('否')),
            _kv(L10n.tr('限流维度'),
          ...
          _section(L10n.tr('时间'), [
            _kv(L10n.tr('时间戳'), Formatters.formatDateTime(log.timestamp), mono: true),
            _kv(L10n.tr('日志 ID'), log.id, mono: true, wrap: true),
          ]),
        ],
      ),
    );
