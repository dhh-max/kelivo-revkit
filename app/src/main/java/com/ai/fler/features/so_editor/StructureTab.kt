    ...
    Column(modifier = modifier.fillMaxSize()) {
        ...
        val searchableOrdinals = remember {
            setOf(
                ...
                StructureSubTab.IMPORTS.ordinal
            )
        }
        ...
        androidx.compose.material3.ScrollableTabRow(
            ...
            Tab(
                selected = selectedSubTab == StructureSubTab.IMPORTS,
                onClick = {
                    viewModel.setStructureSubTab(StructureSubTab.IMPORTS.ordinal)
                    viewModel.loadImports()
                },
                text = { Text("导入") }
            )
            Tab(
                selected = selectedSubTab == StructureSubTab.INIT_ARRAY,
                onClick = {
                    viewModel.setStructureSubTab(StructureSubTab.INIT_ARRAY.ordinal)
                    viewModel.loadInitArray()
                },
                text = { Text("init_array") }
            )
            Tab(
                selected = selectedSubTab == StructureSubTab.ENTRY_POINTS,
                onClick = {
                    viewModel.setStructureSubTab(StructureSubTab.ENTRY_POINTS.ordinal)
                    viewModel.listEntryPoints()
                },
                text = { Text("入口点") }
            )
            Tab(
                selected = selectedSubTab == StructureSubTab.ANTI_DEBUG,
                onClick = {
                    viewModel.setStructureSubTab(StructureSubTab.ANTI_DEBUG.ordinal)
                    viewModel.detectAntiDebug()
                },
                text = { Text("反调试") }
            )
            Tab(
                selected = selectedSubTab == StructureSubTab.DYNAMIC_SECTION,
                onClick = {
                    viewModel.setStructureSubTab(StructureSubTab.DYNAMIC_SECTION.ordinal)
                    viewModel.loadDynamicSection()
                },
                text = { Text("动态节区") }
            )
        }
        ...
        Box(modifier = Modifier.fillMaxSize()) {
            when (selectedSubTab) {
                ...
                StructureSubTab.IMPORTS -> ImportsList(
                    viewModel = viewModel,
                    modifier = Modifier.fillMaxSize()
                )
                StructureSubTab.INIT_ARRAY -> InitArrayList(
                    viewModel = viewModel,
                    onEntryClick = { addr ->
                        scope.launch {
                ...
                StructureSubTab.ENTRY_POINTS -> EntryPointsList(
                    viewModel = viewModel,
                    onEntryClick = { addr ->
                        scope.launch {
                            val paddr = viewModel.resolveJumpAddress(addr)
                ...
                StructureSubTab.ANTI_DEBUG -> AntiDebugPanel(
                    viewModel = viewModel,
                    onFindingClick = { addr ->
                        scope.launch {
                            val paddr = viewModel.resolveJumpAddress(addr)
                ...
                StructureSubTab.DYNAMIC_SECTION -> DynamicSectionPanel(
                    viewModel = viewModel,
                    modifier = Modifier.fillMaxSize()
                )
            }
...
private fun StructureSubTab.label(): String = when (this) {
    ...
    StructureSubTab.IMPORTS -> "导入"
    StructureSubTab.INIT_ARRAY -> "init_array"
    StructureSubTab.ENTRY_POINTS -> "入口点"
    StructureSubTab.ANTI_DEBUG -> "反调试"
    StructureSubTab.DYNAMIC_SECTION -> "动态节区"
}

private enum class StructureSubTab { SECTIONS, SYMBOLS, DYNAMIC_SYMBOLS, FUNCTIONS, STRINGS, IMPORTS, INIT_ARRAY, ENTRY_POINTS, ANTI_DEBUG, DYNAMIC_SECTION }
...
@Composable
private fun ImportsList(
    viewModel: SoEditorViewModel,
    modifier: Modifier = Modifier
) {
    val state by viewModel.importsData.collectAsStateWithLifecycle()
    if (state.isLoading) {
    ...
    if (state.errorMessage != null) {
        Box(modifier = modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
            Text("加载失败: ${state.errorMessage}", style = MaterialTheme.typography.bodySmall,
    ...
    if (state.imports.isEmpty()) {
        Box(modifier = modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
            Text("无导入符号", style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant)
        }
        return
    }
    LazyColumn(modifier = modifier) {
        items(state.imports, key = { it.name + it.address }) { imp ->
            Column(modifier = Modifier.fillMaxWidth().padding(horizontal = 12.dp, vertical = 6.dp)) {
                Text(text = imp.name, style = MaterialTheme.typography.bodySmall,
                    fontFamily = FontFamily.Monospace, maxLines = 1,
                    overflow = androidx.compose.ui.text.style.TextOverflow.Ellipsis)
                Row {
                    Text(text = "0x${imp.address.toString(16).uppercase().padStart(8, '0')}",
                        style = MaterialTheme.typography.labelSmall, fontFamily = FontFamily.Monospace,
                        color = MaterialTheme.colorScheme.onSurfaceVariant)
                    if (imp.type.isNotBlank()) {
                        Spacer(modifier = Modifier.width(8.dp))
                        Text(text = imp.type, style = MaterialTheme.typography.labelSmall,
                            color = MaterialTheme.colorScheme.primary)
                    }
                    if (imp.bind.isNotBlank()) {
                        Spacer(modifier = Modifier.width(8.dp))
                        Text(text = imp.bind, style = MaterialTheme.typography.labelSmall,
                            color = MaterialTheme.colorScheme.onSurfaceVariant)
                    }
                }
            }
...
private fun InitArrayList(
    ...
    val state by viewModel.initArrayData.collectAsStateWithLifecycle()
    if (state.isLoading) {
        Box(modifier = modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
            Text("加载中…", style = MaterialTheme.typography.bodySmall)
        }
    ...
    if (state.entries.isEmpty()) {
        Box(modifier = modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
            Text("无 init/fini array", style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant)
        }
        return
    }
    LazyColumn(modifier = modifier) {
        items(state.entries, key = { it.section + it.index }) { entry ->
            Row(modifier = Modifier.fillMaxWidth().clickable { onEntryClick(entry.vaddr) }
                .padding(horizontal = 12.dp, vertical = 8.dp)) {
                Text(text = entry.section, style = MaterialTheme.typography.labelSmall,
                    color = MaterialTheme.colorScheme.primary, modifier = Modifier.width(100.dp))
                Text(text = "[${entry.index}] 0x${entry.vaddr.toString(16).uppercase()}",
...
@Composable
private fun EntryPointsList(
    viewModel: SoEditorViewModel,
    onEntryClick: (Long) -> Unit,
    modifier: Modifier = Modifier
) {
    val state by viewModel.entryPointsData.collectAsStateWithLifecycle()
    if (state.isLoading) {
        Box(modifier = modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
            Text("加载中…", style = MaterialTheme.typography.bodySmall)
        }
    ...
    if (state.entries.isEmpty()) {
        Box(modifier = modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
            Text("无入口点", style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant)
        }
        return
    }
    LazyColumn(modifier = modifier) {
        items(state.entries, key = { it.type + it.address }) { entry ->
            Column(modifier = Modifier.fillMaxWidth()
                .clickable { onEntryClick(entry.address) }
                .padding(horizontal = 12.dp, vertical = 8.dp)) {
                Row {
                    Text(text = entry.type, style = MaterialTheme.typography.labelSmall,
                        color = MaterialTheme.colorScheme.primary, modifier = Modifier.width(100.dp))
                    Text(text = "0x${entry.address.toString(16).uppercase()}",
                        style = MaterialTheme.typography.bodySmall, fontFamily = FontFamily.Monospace)
                    entry.section?.let {
                        Spacer(modifier = Modifier.width(8.dp))
...
@Composable
private fun AntiDebugPanel(
    viewModel: SoEditorViewModel,
    onFindingClick: (Long) -> Unit,
    modifier: Modifier = Modifier
) {
    val state by viewModel.antiDebugData.collectAsStateWithLifecycle()
    if (state.isLoading) {
        Box(modifier = modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
    ...
    if (state.errorMessage != null) {
        Box(modifier = modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
            Text("扫描失败: ${state.errorMessage}", style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.error)
        }
        return
    }
    ...
    if (result == null || result.findings.isEmpty()) {
        Box(modifier = modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
            Text(result?.note ?: "点击 Tab 重新扫描", style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant)
        }
        return
    }
    LazyColumn(modifier = modifier) {
        item {
            Text(text = "发现 ${result.categoryCount} 类反调试模式",
                style = MaterialTheme.typography.titleSmall,
                modifier = Modifier.padding(horizontal = 12.dp, vertical = 8.dp))
        }
        items(result.findings, key = { it.category + it.label }) { finding ->
            Column(modifier = Modifier.fillMaxWidth().padding(horizontal = 12.dp, vertical = 6.dp)) {
                Row {
                    Text(text = finding.category, style = MaterialTheme.typography.labelSmall,
                        color = MaterialTheme.colorScheme.primary, modifier = Modifier.width(80.dp))
                    Text(text = finding.label, style = MaterialTheme.typography.bodySmall,
                        fontWeight = FontWeight.Bold)
                    Spacer(modifier = Modifier.width(8.dp))
                    Text(text = "(${finding.matchCount})", style = MaterialTheme.typography.labelSmall,
                        color = MaterialTheme.colorScheme.onSurfaceVariant)
                }
                finding.matches.take(10).forEach { match ->
                    Row(modifier = Modifier.fillMaxWidth()
                        .clickable { onFindingClick(match.address) }
                        .padding(vertical = 2.dp)) {
                        Text(text = "  0x${match.address.toString(16).uppercase()}",
                            style = MaterialTheme.typography.labelSmall, fontFamily = FontFamily.Monospace,
                            color = MaterialTheme.colorScheme.onSurfaceVariant)
                        Spacer(modifier = Modifier.width(8.dp))
                        Text(text = match.info.take(80), style = MaterialTheme.typography.labelSmall,
                            maxLines = 1, overflow = androidx.compose.ui.text.style.TextOverflow.Ellipsis)
                    }
                }
                if (finding.matches.size > 10) {
                    Text(text = "  …还有 ${finding.matches.size - 10} 条",
                        style = MaterialTheme.typography.labelSmall,
                        color = MaterialTheme.colorScheme.onSurfaceVariant)
                }
            }
...
@Composable
private fun DynamicSectionPanel(
    viewModel: SoEditorViewModel,
    modifier: Modifier = Modifier
) {
    val state by viewModel.dynamicSectionData.collectAsStateWithLifecycle()
    if (state.isLoading) {
        Box(modifier = modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
            Text("加载中…", style = MaterialTheme.typography.bodySmall)
        }
    ...
    if (state.content == null) {
        Box(modifier = modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
            Text("无动态节区数据", style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant)
        }
        return
    }
    Column(modifier = modifier.padding(12.dp)) {
        Text(text = "格式: ${state.format}", style = MaterialTheme.typography.labelSmall,
            color = MaterialTheme.colorScheme.primary)
        Spacer(modifier = Modifier.height(8.dp))
        Text(text = state.content ?: "", style = MaterialTheme.typography.bodySmall,
            fontFamily = FontFamily.Monospace, overflow = androidx.compose.ui.text.style.TextOverflow.Ellipsis)
    }
}
